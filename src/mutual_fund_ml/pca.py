...
def get_pca_model_path() -> Path:
    """Returns the file path for the saved PCA model."""
    config = load_config()
    art_dir = config["paths"]["artifact_dir"]
    return get_resolved_path(art_dir) / "models" / "pca_model.joblib"

def save_pca_artifacts(pca: PCA, df_loadings: pd.DataFrame, df_variance: pd.DataFrame, df_scores: pd.DataFrame) -> None:
    ...
    import json
    with open(meta_dir / "pca_input_features.json", "w", encoding="utf-8") as f:
        json.dump(features_list, f, indent=2)
    logger.info(f"PCA input features metadata saved to: {meta_dir / 'pca_input_features.json'}")

    # Save CSV outputs
    pca_out_dir = get_resolved_path(out_dir) / "pca"
    pca_out_dir.mkdir(parents=True, exist_ok=True)

    df_loadings.to_csv(pca_out_dir / "pca_loadings.csv")
    df_variance.to_csv(pca_out_dir / "explained_variance.csv", index=False)
    df_scores.to_csv(pca_out_dir / "pca_scores.csv", index=False)
    logger.info(f"PCA loadings, variance table, and scores exported to: {pca_out_dir}")
